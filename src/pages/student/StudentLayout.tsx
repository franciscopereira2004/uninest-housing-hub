...
import { Heart, LayoutDashboard, Loader2, MessageSquare, UserCircle } from "lucide-react";
...
const navItems = [
  ...
  { to: "/student/saved", label: "Anúncios guardados", icon: Heart },
  { to: "/student/messages", label: "Mensagens", icon: MessageSquare },
  { to: "/student/profile", label: "Perfil", icon: UserCircle }
];
        ...
        <aside className="hidden h-full w-72 shrink-0 border-r border-border/70 bg-background/95 md:flex md:flex-col">
          <nav className="flex-1 space-y-2 p-4">
            {navItems.map(({ to, label, icon: Icon, end }) => {
              const showBadge = to === "/student/messages" && unread > 0;
              return (
                <NavLink
                  key={to}
                  to={to}
